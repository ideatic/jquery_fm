 ...
 */
class jExplorer_FileProviderFS extends jExplorer_FileProvider
{
    ...
    protected function _populate_file_item($path, $folder)
    {
        ...
        if ($item->is_folder) {
            ...
            $item->info = jExplorer_FileManagerHelper::format_size($item->size);
    ...
    public function delete(FileManagerItem $file)
    {
        ...
        if ($file->is_folder) {
            return is_dir($file->path) && jExplorer_FileManagerHelper::recursive_delete($file->path);
        } else {
