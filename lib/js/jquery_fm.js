...
(function ($) {
    ...
    Plugin.prototype._createFile = function (fileData) {
        ...
        if (fileData['is_folder']) {
            ...
            onclick = function () {
                //Download file
                var fileUrl = plugin._options['ajax_endpoint'] + (plugin._options['ajax_endpoint'].indexOf('?') == -1 ? '?' : '&') + 'action=download&file=' + fileData['name'];
            ...
            for (var i in folders) {
                ...
                if (folder.length) {
                    if (i == folders.length - 1) {//Last folder
                        ...
                        var folderPath = folders.slice(0, i).join('/');
                        var $a = $('<a />')
                            .text(folder)
                            .click(function () {
                                plugin.navigateTo(folderPath);
                                return false;
                            });
