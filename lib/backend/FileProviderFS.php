 ...
 */
class FileProviderFS extends FileProvider
{
    ...
    protected function _populate_file_item($path, $folder)
    {
        ...
        if ($item->is_folder) {
            ...
            $item->info = FileManagerHelper::format_size($item->size);
    ...
    public function delete(FileManagerItem $file)
    {
        ...
        if ($file->is_folder) {
            return is_dir($file->path) && FileManagerHelper::recursive_delete($file->path);
        } else {
